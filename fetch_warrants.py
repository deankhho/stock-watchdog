#!/usr/bin/env python3
"""
fetch_warrants.py — 認售權證標的清單 → data/warrants.json（§8 追加項目）
...
2026-08-16 使用者追問「只要有或沒有就好不行？」，重新查證後找到可用組合：
  ...
  1. TWSE openapi t187ap37_L（權證發行資訊）：含「標的證券/指數」欄，但只有名稱、
     是全部歷史發行（含已到期），且沒有代號欄。過濾「權證類型=認售」（只有認售/PUT
     跟放空同方向，認購/CALL 無關）＋「履約截止日 >= 今天」才是目前有效的。
...
只保留 predict_in／edge 展開列用得到的判斷：該代號目前是否有仍在有效期間內的認售權證。
...
ABS_FLOOR = 50               # 唯一觀測值 2026-08-16 為 180 檔，這是保守猜的下限，
...
def fetch_active_put_underlyings() -> set:
    """→ 目前仍在履約期間內的認售權證標的名稱集合。"""
    ...
    for x in data:
        ...
        if exp and exp >= today:
            names.add(x.get("標的證券/指數", ""))
    names.discard("")
    return names


def resolve_codes(fetch_isin=fetch_isin_name_to_code, fetch_put=fetch_active_put_underlyings) -> tuple:
    """→ (codes: set[str], matched: int, unmatched_names: set[str])。
    unmatched 多半是指數／ETF／期貨等非個股標的，不是 bug，但保留供健檢／除錯用。"""
    name_to_code = fetch_isin()
    put_names = fetch_put()
    codes = {name_to_code[n] for n in put_names if n in name_to_code}
    unmatched = put_names - set(name_to_code.keys())
    return codes, len(codes), unmatched
...
def run(resolve_fn=resolve_codes, prev: dict = None) -> dict:
    ...
    try:
        codes, count, _unmatched = resolve_fn()
...
def main():
    ...
    print(f"完成：認售權證標的 {out.get('count', 0)} 檔，state={out['state']} → {OUT}")
...
def selftest():
    ...
    accept, sev, reason = classify_fetch(10)
    assert not accept and "絕對地板" in reason, (accept, sev, reason)
    accept, sev, reason = classify_fetch(180)
    assert accept and sev == "ok", (accept, sev, reason)

    # === 3. fetch_active_put_underlyings()：只認「認售」＋履約截止日未過 ===
    fake_data = [
        {"權證類型": "認售", "標的證券/指數": "A股", "履約截止日": "1150901"},   # 有效
        {"權證類型": "認購", "標的證券/指數": "B股", "履約截止日": "1150901"},   # 認購，排除
        {"權證類型": "認售", "標的證券/指數": "C股", "履約截止日": "1140101"},   # 已過期，排除
    ...
    try:
        ...
        for x in fake_data:
            ...
            if exp and exp >= today:
                names.add(x["標的證券/指數"])
        assert names == {"A股"}, names          # B股(認購)、C股(過期) 都要被排除
    finally:
        requests.get = orig_get

    # === 4. resolve_codes()：name→code 橋接，含無法比對的名稱不拋例外 ===
    def fake_isin():
        return {"A股": "1111", "B股": "2222"}

    def fake_put():
        return {"A股", "C股（查無此代號，指數型）"}

    codes, count, unmatched = resolve_codes(fake_isin, fake_put)
    assert codes == {"1111"}, codes
    assert count == 1, count
    assert unmatched == {"C股（查無此代號，指數型）"}, unmatched

    # === 5. run()：截斷 → 沿用舊清單，不可寫入殘缺資料 ===
    prev_good = {"state": "ok", "fetched_at": "2026-08-10", "reason": None,
                "codes": ["1234", "5678"], "count": 2, "count_history": []}
    ...
    assert out["codes"] == ["1234", "5678"], out       # 🔴 舊清單原封不動
    assert out["fetched_at"] == "2026-08-10", out

    # === 6. run()：正常情況 + 抓取拋例外都測過 ===
    def resolve_ok():
        return {f"{1000+i}" for i in range(180)}, 180, set()

    out = run(resolve_ok, prev={})
    assert out["state"] == "ok" and out["count"] == 180, out
    assert out["codes"] == sorted(out["codes"]), out
    ...
    assert out["codes"] == ["1234", "5678"], out
